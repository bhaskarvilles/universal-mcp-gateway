    ...
    fn validate_args(&self, args: &[String]) -> Result<(), AdapterError> {
        ...
        for arg in args {
            if !self
                .allowed_args
                .iter()
                .any(|allowed| arg.starts_with(allowed))
            {
...
impl Adapter for CLIAdapter {
    ...
    async fn execute(
        ...
        let output = tokio::time::timeout(timeout, cmd.output())
            ...
            .map_err(|e| {
                AdapterError::ExecutionError(format!("Failed to execute command: {}", e))
            })?;
